                        ...
                        <Stack spacing="xs">
                          {previewProducts.length > 0 && (
                            <Group position="apart">
                              <Checkbox 
                                label={`Chọn tất cả (${previewProducts.filter((p: any) => !p.productPromotion).length} sản phẩm có sẵn)`}
                                checked={checkedProductIds.size === previewProducts.filter((p: any) => !p.productPromotion).length && previewProducts.filter((p: any) => !p.productPromotion).length > 0}
                                indeterminate={checkedProductIds.size > 0 && checkedProductIds.size < previewProducts.filter((p: any) => !p.productPromotion).length}
                          ...
                          <div style={{ maxHeight: '420px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '8px' }}>
                            {[...previewProducts]
                              .sort((a: any, b: any) => {
                                const aPromo = a.productPromotion ? 1 : 0;
                                const bPromo = b.productPromotion ? 1 : 0;
                                return aPromo - bPromo;
                              })
                              .map((p: any) => {
                                const isDisabled = p.productPromotion !== null && p.productPromotion !== undefined;
                                return (
                                  <Group key={p.productId} position="apart" p="xs" style={{ background: '#fff', borderRadius: '8px', border: '1px solid #e9ecef', opacity: isDisabled ? 0.6 : 1 }}>
                                    ...
                                    <Badge size="xs" color={isDisabled ? "red" : "green"}>{isDisabled ? "Đang có KM khác" : "Sẵn sàng"}</Badge>
                                  </Group>
                                );
                              })}
                          </div>
                    ...
                    {!isLoadingAllProducts && allProductsList.length === 0 ? (
                      ...
                      <Stack spacing="xs">
                        {allProductsList.length > 0 && (
                          <Group position="apart">
                            <Checkbox 
                              label={`Chọn tất cả (${allProductsList.filter((p: any) => !p.productPromotion).length} sản phẩm có sẵn)`}
                              checked={checkedTabProductIds.size === allProductsList.filter((p: any) => !p.productPromotion).length && allProductsList.filter((p: any) => !p.productPromotion).length > 0}
                              indeterminate={checkedTabProductIds.size > 0 && checkedTabProductIds.size < allProductsList.filter((p: any) => !p.productPromotion).length}
