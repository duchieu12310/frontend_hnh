...
function ProductVariantRow({
  ...
  const handleVariantQuantityInput = (quantity: number, index: number) => {
    setVariants(produce(variants, draft => {
    ...
    <tr className={`border-b border-gray-200 dark:border-gray-700 ${
      ...
      <td className="px-3 py-2 w-20">
        <input
          ...
          onChange={(e) => handleVariantQuantityInput(Number(e.target.value) || 0, index)}
          disabled={isDisabled}
          min={0}
          className="w-full px-2 py-1 text-xs border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 disabled:opacity-50 disabled:cursor-not-allowed focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
        />
      </td>
